...
export function renderMonthChange(pct) {
  ...
  // 确保标签元素存在
  let label = el.querySelector(".month-change-label");
  if (!label) {
    label = document.createElement("span");
    label.className = "month-change-label";
    el.insertBefore(label, el.firstChild);
  }
  label.textContent = t("monthChangeLabel");

  // 构建滚动文本（符号 + 数字 + % ）
  const sign = pct >= 0 ? "+" : "";
  const text = `${sign}${pct.toFixed(2)}%`;
  applyRollingText(el, text, 1); // startIndex=1 跳过 label 元素
}
